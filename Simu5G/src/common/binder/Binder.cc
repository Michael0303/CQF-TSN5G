...
void Binder::registerCarrier(double carrierFrequency, unsigned int carrierNumBands, unsigned int numerologyIndex, bool useTdd, unsigned int tddNumSymbolsDl, unsigned int tddNumSymbolsUl) {
    ...
    if (it != componentCarriers_.end() && carrierNumBands <= componentCarriers_[carrierFrequency].numBands) {
        ...
        EV << "Binder::registerCarrier - Registered carrier @ " << carrierFrequency << "GHz with " << carrierNumBands << " bands and numerology index " << numerologyIndex << endl;
...
SlotFormat Binder::computeSlotFormat(bool useTdd, unsigned int tddNumSymbolsDl, unsigned int tddNumSymbolsUl) {
    ...
    // print out sf
    EV << "Binder::computeSlotFormat - sf.tdd " << sf.tdd << " sf.numDlSymbols " << sf.numDlSymbols << " sf.numUlSymbols " << sf.numUlSymbols << " sf.numFlexSymbols " << sf.numFlexSymbols << endl;
    return sf;
}
...
bool Binder::checkD2DCapability(MacNodeId src, MacNodeId dst) {
    if (src < UE_MIN_ID || (src >= macNodeIdCounter_[1] && src < NR_UE_MIN_ID) || src >= macNodeIdCounter_[2] || dst < UE_MIN_ID || (dst >= macNodeIdCounter_[1] && dst < NR_UE_MIN_ID) || dst >= macNodeIdCounter_[2])
